public class HomeWork {
    ...
    + 1) Создаем класс военкомат model.MilitaryOffice
    2) Создать в классе model.MilitaryOffice конструктор, который принимает model.PersonRegistry
    3) model.PersonRegistry содержит список людей (model.Person), которые могут быть призваны в армию
    + 4) Создать класс model.Person, который будет содержать следующие поля имя, возраст, пол, адрес(страна, город)
    Военкомат должен уметь:
    a) вывести имена всех людей годных к военной службе на текущий момент времени, которые есть в model.PersonRegistry
    T.е нужно написать метод в классе model.MilitaryOffice, который будет отфильтровывать годных к службе призывников
    ...
    5) Создать класс Task1.Main, наполнить model.PersonRegistry людьми, по желанию можно создавать людей через консоль
    */
